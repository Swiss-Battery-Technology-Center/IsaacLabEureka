...
def main(args_cli):
    ...
    if args_cli.rl_library == "rsl_rl":
        play_rsl_rl(env, args_cli, simulation_app, device, task, checkpoint)

    elif args_cli.rl_library == "rl_games":
        play_rl_games(env, args_cli, simulation_app, device, task, checkpoint)
        
    elif args_cli.rl_library == "skrl":
        play_skrl(env, args_cli, simulation_app, device, task, checkpoint)
            
    env.close()
    simulation_app.close()


def play_rsl_rl(env, args_cli, simulation_app, device, task, checkpoint):
...
def play_rl_games(env, args_cli, simulation_app, device, task, checkpoint):
    from isaaclab_tasks.utils.parse_cfg import load_cfg_from_registry
...
def play_skrl(env, args_cli, simulation_app, device, task, checkpoint):
    from isaaclab_tasks.utils.parse_cfg import load_cfg_from_registry
    import time
